...
class Product(models.Model):
    ...
    # if user is deleted, set product to null
    ...
    # automatically set the date when the product is created
    _id = models.AutoField(primary_key=True, editable=False)
    # automatically set the id and make it not editable
...
class Review(models.Model):
    ...
    # if product is deleted, set review to null
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True)
    name = models.CharField(max_length=200, null=True, blank=True)
...
class Order(models.Model):
    ...
    # automatically set the date when the order is paid
    isDelivered = models.BooleanField(default=False)
    deliveredAt = models.DateTimeField(
        auto_now_add=False, null=True, blank=True)
    # automatically set the date when the order is delivered
    createdAt = models.DateTimeField(auto_now_add=True)
    _id = models.AutoField(primary_key=True, editable=False)
...
class OrderItem(models.Model):
    ...
    # if order is deleted, set order item to null
    name = models.CharField(max_length=200, null=True, blank=True)
